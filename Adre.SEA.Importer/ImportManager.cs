            ...
            using (ASEAContext ctx = new ASEAContext())
            {
                using (WSLSSContext ctx2 = new WSLSSContext())
                {
                    ...
                    foreach (var wslContingent in wslContingents)
                    {
                        ...
                        contingents.AddOrUpdate(contingent => contingent.WslId, newContingent);
                    ...
                    foreach (var wslAthlete in wslAthletes)
                    {
                        ...
                        var newAthlete = new Adre.SEA.Database.Athlete
                        {
                            ...
                            //SeasonBest = wslAthlete.SeasonBest,
                            //SeasonBestDate = wslAthlete.SeasonBestDate,
                        ...
                        if ((existingAthlete != null))
                        {
                            ...
                            newAthlete.BibNo = existingAthlete.BibNo;
                            newAthlete.PreferredName = existingAthlete.PreferredName;
                            newAthlete.FullName = existingAthlete.FullName;
                        }

                        athletes.AddOrUpdate(athlete => athlete.WslId, newAthlete);
                    }
                    ...
                    foreach (var wslEvent in wslEvents)
                    {
                        ...
                        var newEvent = new Adre.SEA.Database.Event
                        {
                            ...
                            //AseanRecord = wslEvent.AR_Rec,
                            //AseanRecordHolder = wslEvent.AR_Holder,
                            //AseanRecordYear = wslEvent.AR_Year,
                            //AseanRecordVenue = wslEvent.AR_Venue,
                            //SeaGamesRecord = wslEvent.MR_Rec,
                            //SeaGamesRecordHolder = wslEvent.MR_Holder,
                            //SeaGamesRecordVenue = wslEvent.MR_Venue,
                            //SeaGamesRecordYear = wslEvent.MR_Year,
                            Athletes = ctx.Athletes.Where(ath => aa.Contains(ath.WslId)).ToList()
                        };
